...
- **颜色主题**：5 套内置配色一键切换（默认 / 红黄粉 / 黄绿灰 / 蓝绿灰 / 黑白灰），圆形象色块选取
- **对比度滑块**：0%~100% 自由调节卡片与背景的明暗差异
...
统一 Material 3 深色主题，所有页面使用灰色胶囊卡片布局，适配深色模式蓝灰色调。
内置 5 套配色（默认暖金、红黄粉、黄绿灰、蓝绿灰、黑白灰），支持对比度滑块实时调整卡片明暗差异。
